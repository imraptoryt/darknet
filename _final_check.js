...
const TRANSLATIONS = {
  ...
  fr: {
    ...
    fieldAvatarColor: "Couleur d'avatar",
    thAvatar: "Avatar",
...
function renderAccountsTable(){
  ...
  state.accounts
    ...
    .forEach(acc=>{
      ...
      const swatchBtn = el(`<button class="swatch-btn" style="background:${av.color};" title="${escapeHtml(av.letter)}"></button>`);
      if(canManage){ swatchBtn.onclick = ()=> editAvatarColor(acc); }
      else { swatchBtn.disabled = true; swatchBtn.style.cursor = 'default'; }
      avatarTd.appendChild(swatchBtn);
...
async function editAvatarColor(acc){
  const picker = document.createElement('input');
  picker.type = 'color';
  picker.value = acc.avatar_color || avatarFor(acc).color || '#00ff9c';
  picker.style.position = 'fixed';
  picker.style.opacity = '0';
  document.body.appendChild(picker);
  picker.addEventListener('change', async ()=>{
    const { error } = await sb.from('profiles').update({ avatar_color: picker.value }).eq('id', acc.id);
...
function renderMessages(msgs, threadId){
  ...
  msgs.forEach(m=>{
    ...
    let bubbleCls = 'msg';
    if(fullView){
      bubbleCls += m.visibility === 'internal' ? ' internal' : ' client-reply';
    } else {
      bubbleCls += mine ? ' from-client' : ' client-reply';
    }
    ...
    const row = el(`<div class="${rowCls}">
      <div class="avatar ${grouped?'spacer':''}" style="background:${color};">${escapeHtml(letter)}</div>
      <div class="msg-col">
        <div class="${bubbleCls}">
          <div class="meta"><span class="cat-dot" style="background:${color};"></span>${senderLabel} · ${fmtTime(m.created_at)}</div>
          <div class="body"></div>

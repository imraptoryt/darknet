...
const TRANSLATIONS = {
  en: {
    ...
    loginUsername: "Username", loginPassword: "Password", loginBtn: "Connect", loginConnecting: "Authenticating...",
    ...
    noChatsYet: "(no chats yet)", empty: "(empty)",
    tabAccounts: "Accounts", tabRoles: "Roles", tabCategories: "Categories",
    createAccountTitle: "Create Account",
    fieldUsername: "Username", fieldPassword: "Password",
    fieldCategory: "Category (their personal chat)", fieldSubcategory: "Sub-category (optional)",
    fieldRole: "Role (optional)", roleDefaultOption: "— Member (default) —",
    createBtn: "Create", allAccountsTitle: "All Accounts",
    ...
    createCategoryTitle: "Create Category", editCategoryTitle: "Edit Category",
    fieldCategoryName: "Name", fieldColor: "Color", thColor: "Color", thWebhookCol: "Webhook",
    fieldWebhook: "Discord webhook for this category (optional)",
    webhookNote: "Leave blank to use the default DISCORD_WEBHOOK_URL for this category's notifications.",
    fieldAvatarColor: "Avatar color", thAvatar: "Avatar",
    imageBtn: "🖼", imageOnlyPng: "Only .png images are supported.", imageTooBig: "Image is too large (max 5MB).", uploading: "Uploading...",
    createRoleTitle: "Create Role", editRoleTitle: "Edit Role",
    ...
    createRoleBtn: "Create Role", saveChangesBtn: "Save Changes",
    ...
    editBtn: "Edit",
    addSubcatTitle: "Add Sub-category", fieldParentCategory: "Parent Category", fieldSubcatName: "Name", addBtn: "Add",
    categoriesTitle: "Categories", thCategoryCol: "Category", thSubcategories: "Sub-categories",
    ...
    sendBtn: "Send", readOnlyNote: "View only — replying here requires level 6+.",
    myChat: "💬 My Chat", settingsNav: "⚙ Settings",
    yes: "yes", no: "no", internalNote: "(internal note)", sentToClient: "(sent to client)", you: "You", staff: "Staff",
    editAccountTitle: "Edit Account", fieldNewPassword: "New password (leave blank to keep current)",
    maintenanceTitle: "Maintenance", cleanupImagesBtn: "Clear images older than 5 days",
    cleanupNote: "Images are also cleared automatically every day by a scheduled job.",
    cleanupDone: "Done — {n} image(s) cleared.",
    savedOk: "Saved.",
  },
  fr: {
    appTitle: "TERMINAL D'ACCÈS SÉCURISÉ",
    appSubtitle: "PERSONNEL AUTORISÉ UNIQUEMENT // ENTREZ VOS IDENTIFIANTS",
    loginUsername: "Nom d'utilisateur", loginPassword: "Mot de passe", loginBtn: "Connexion", loginConnecting: "Authentification...",
    ...
    noChatsYet: "(aucun chat pour l'instant)", empty: "(vide)",
    tabAccounts: "Comptes", tabRoles: "Rôles", tabCategories: "Catégories",
    createAccountTitle: "Créer un compte",
    fieldUsername: "Nom d'utilisateur", fieldPassword: "Mot de passe",
    fieldCategory: "Catégorie (son chat personnel)", fieldSubcategory: "Sous-catégorie (optionnel)",
    fieldRole: "Rôle (optionnel)", roleDefaultOption: "— Membre (par défaut) —",
    createBtn: "Créer", allAccountsTitle: "Tous les comptes",
    ...
    createCategoryTitle: "Créer une catégorie", editCategoryTitle: "Modifier la catégorie",
    fieldCategoryName: "Nom", fieldColor: "Couleur", thColor: "Couleur", thWebhookCol: "Webhook",
    fieldWebhook: "Webhook Discord pour cette catégorie (optionnel)",
    webhookNote: "Laisser vide pour utiliser le DISCORD_WEBHOOK_URL par défaut pour cette catégorie.",
    fieldAvatarColor: "Couleur d'avatar", thAvatar: "Avatar",
    imageBtn: "🖼", imageOnlyPng: "Seules les images .png sont supportées.", imageTooBig: "Image trop volumineuse (5 Mo max).", uploading: "Envoi en cours...",
    createRoleTitle: "Créer un rôle", editRoleTitle: "Modifier le rôle",
    ...
    createRoleBtn: "Créer le rôle", saveChangesBtn: "Enregistrer",
    ...
    editBtn: "Modifier",
    addSubcatTitle: "Ajouter une sous-catégorie", fieldParentCategory: "Catégorie parente", fieldSubcatName: "Nom", addBtn: "Ajouter",
    categoriesTitle: "Catégories", thCategoryCol: "Catégorie", thSubcategories: "Sous-catégories",
    ...
    sendBtn: "Envoyer", readOnlyNote: "Lecture seule — répondre ici nécessite le niveau 6+.",
    myChat: "💬 Mon Chat", settingsNav: "⚙ Paramètres",
    yes: "oui", no: "non", internalNote: "(note interne)", sentToClient: "(envoyé au client)", you: "Vous", staff: "Staff",
    editAccountTitle: "Modifier le compte", fieldNewPassword: "Nouveau mot de passe (laisser vide pour garder l'actuel)",
    maintenanceTitle: "Maintenance", cleanupImagesBtn: "Effacer les images de plus de 5 jours",
    cleanupNote: "Les images sont aussi effacées automatiquement chaque jour par une tâche planifiée.",
    cleanupDone: "Terminé — {n} image(s) effacée(s).",
    savedOk: "Enregistré.",
  }
};
...
function applyTranslations(){
  ...
  document.querySelectorAll('[data-i18n-placeholder]').forEach(elx=>{ elx.setAttribute('placeholder', t(elx.getAttribute('data-i18n-placeholder'))); });
...
(function matrixRain(){
  ...
  function resize(){ w = canvas.width = window.innerWidth; h = canvas.height = window.innerHeight; cols = Math.floor(w/16); drops = new Array(cols).fill(1); }
  window.addEventListener('resize', resize);
  resize();
  function draw(){
    ctx.fillStyle = 'rgba(6,9,9,0.08)'; ctx.fillRect(0,0,w,h);
...
function runBootSequence(done){
  ...
  function next(){
    if(i >= lines.length){
      setTimeout(()=>{ document.getElementById('boot-screen').classList.add('hidden'); setTimeout(done, 550); }, 300);
      return;
    }
...
function fmtDay(ts){
  ...
  if(sameDay) return state.lang==='fr' ? "Aujourd'hui" : 'Today';
  if(d.toDateString() === yesterday.toDateString()) return state.lang==='fr' ? 'Hier' : 'Yesterday';
  return d.toLocaleDateString();
}
// "undead_vikings" / "MC - Undead Vikings" -> "Undead Vikings" — strips - and _,
// collapses whitespace, capitalizes the first letter of each word.
...
// lighten/darken a hex color, percent in [-1,1]
...
function avatarStyle(hex){
  const light = shadeColor(hex, 0.35), dark = shadeColor(hex, -0.25);
  return `background:linear-gradient(135deg, ${light}, ${dark}); box-shadow:0 0 10px ${hex}66, inset 0 0 0 1.5px rgba(255,255,255,0.18);`;
}
...
const state = {
  ...
  session: null, profile: null, role: null, level: 0,
  mode: 'solo', // 'full' | 'scoped' | 'solo'
  grantedCategoryIds: [],
  roles: [], categories: [], subcategories: [], roleCategories: [],
  threads: [], accounts: [],
  myThreadId: null,
  activeCategoryId: null, activeThreadId: null,
  editingRoleId: null, editingCategoryId: null,
  msgChannel: null,
};
...
async function attemptLogin(){
  ...
  errEl.textContent = '';
  if(!username || !password){ errEl.textContent = t('errFields'); return; }
  btn.disabled = true; btn.textContent = t('loginConnecting');
  const email = usernameToEmail(username);
  const { data, error } = await sb.auth.signInWithPassword({ email, password });
  btn.disabled = false; btn.textContent = t('loginBtn');
  if(error){ errEl.textContent = t('errInvalidCreds'); return; }
  await onAuthed(data.session);
}

async function onAuthed(session){
  state.session = session;
  const { data: profile, error } = await sb.from('profiles').select('*, roles(*)').eq('id', session.user.id).single();
  if(error || !profile){ document.getElementById('login-error').textContent = t('errNoProfile'); await sb.auth.signOut(); return; }
  if(!profile.is_active){ document.getElementById('login-error').textContent = t('errDisabled'); await sb.auth.signOut(); return; }
  state.profile = profile; state.role = profile.roles || null; state.level = state.role ? state.role.level : 0;
...
async function loadApp(){
  ...
  document.getElementById('who-level').textContent = (state.role ? state.role.name + ' — lvl ' + state.role.level : '—');

  await refreshAllData();
  applyTranslations();
  renderSidebar();

  if(state.mode === 'solo' && state.myThreadId) openThread(state.myThreadId);
}
...
async function refreshAllData(){
  ...
  state.categories = ((results[0].data) || []).slice().sort((a,b)=> CATEGORY_ORDER.indexOf(a.name) - CATEGORY_ORDER.indexOf(b.name));
  ...
  if(state.mode === 'full'){
    ...
    state.roles = []; state.accounts = []; state.roleCategories = [];
  }
...
/* ============================================================================
   AVATARS / NAMES
   ========================================================================== */
function categoryForAccountId(accId){
  const th = state.threads.find(t=>t.client_id===accId);
  if(!th) return null;
  return state.categories.find(c=>c.id===th.category_id) || null;
}
function avatarFor(accLike){
  const cat = categoryForAccountId(accLike.id);
  const color = accLike.avatar_color || (cat ? cat.color : null) || '#00ff9c';
  let letter;
  if(cat) letter = cat.name.charAt(0).toUpperCase();
  else if(accLike.roles && accLike.roles.name) letter = accLike.roles.name.charAt(0).toUpperCase();
  else letter = (accLike.username || '?').charAt(0).toUpperCase();
  return { color, letter };
}
function avatarForMessage(m, threadId, fullView){
  if(fullView){
    const acc = (m.sender_id === state.profile.id) ? state.profile : (state.accounts.find(a=>a.id===m.sender_id) || { id:m.sender_id, username:m.sender_username });
    return avatarFor(acc);
  }
  const mine = m.sender_id === state.profile.id;
  if(mine) return avatarFor(state.profile);
  const th = state.threads.find(t=>t.id===threadId);
  const cat = th ? state.categories.find(c=>c.id===th.category_id) : null;
  return { color: cat ? cat.color : '#39d0ff', letter: cat ? cat.name.charAt(0).toUpperCase() : 'S' };
}
...
function renderSidebar(){
  ...
  const visibleCategories = state.mode === 'full' ? state.categories : state.categories.filter(c => state.grantedCategoryIds.includes(c.id));
...
function threadLabel(th){
  ...
  if(acc) return prettyName(acc.username);
  if(th.client_id === state.profile.id) return prettyName(state.profile.username);
  return prettyName(th.title) || 'chat';
}
...
function renderSettingsRefs(){
  ...
  document.getElementById('new-subcat-parent').innerHTML = state.categories.map(c=>`<option value="${c.id}">${escapeHtml(c.name)}</option>`).join('');

  const grid = document.getElementById('new-role-categories');
  grid.innerHTML = state.categories.map(c=>`<label><input type="checkbox" class="role-cat-check" value="${c.id}"> ${escapeHtml(c.name)}</label>`).join('');
}
...
function updateNewAccSubcats(){
  ...
  document.getElementById('new-acc-subcategory').innerHTML = '<option value="">—</option>' + subs.map(s=>`<option value="${s.id}">${escapeHtml(s.name)}</option>`).join('');
}
...
function renderSettingsAll(){
  ...
  document.getElementById('maintenance-card').style.display = canManage ? '' : 'none';
  document.getElementById('create-category-card').style.display = canManage ? '' : 'none';
...
function renderAccountsTable(){
  ...
  tbody.innerHTML = '';
  const canManage = state.level >= 9;
  state.accounts.slice().sort((a,b)=> (a.username<b.username?-1:1)).forEach(acc=>{
    ...
    const av = avatarFor(acc);
    const row = el(`<tr>
      <td></td>
      <td>${escapeHtml(prettyName(acc.username))}</td>
    ...
    const avatarTd = row.children[0];
    const swatch = el(`<span class="swatch-btn" style="${avatarStyle(av.color)}" title="${escapeHtml(av.letter)}"></span>`);
    avatarTd.appendChild(swatch);
    const actionsTd = row.querySelector('.row-actions');
    if(canManage){
      const editBtn = el(`<button class="btn secondary small">${escapeHtml(t('editBtn2'))}</button>`);
      editBtn.onclick = ()=> openEditAccountModal(acc);
      actionsTd.appendChild(editBtn);
      if(acc.id !== state.profile.id){
    ...
    if(actionsTd.children.length === 0){ actionsTd.innerHTML = '<span style="color:var(--dim)">—</span>'; }
...
function renderRolesTable(){
  ...
  state.roles.forEach(r=>{
    const grantedNames = state.roleCategories.filter(rc=>rc.role_id===r.id)
      .map(rc=>{ const c = state.categories.find(cc=>cc.id===rc.category_id); return c?c.name:null; }).filter(Boolean);
...
function renderCategoriesTable(){
  ...
  state.categories.forEach(c=>{
    ...
    const webhookLabel = c.webhook_url ? '✅' : '—';
    const row = el(`<tr><td>${swatch}</td><td>${escapeHtml(c.name)}</td><td>${escapeHtml(subs)}</td><td>${webhookLabel}</td><td></td></tr>`);
    if(canManage){
      const editBtn = el(`<button class="btn secondary small">${escapeHtml(t('editBtn2'))}</button>`);
      editBtn.onclick = ()=> startEditCategory(c);
      row.lastElementChild.appendChild(editBtn);
    } else {
      row.lastElementChild.innerHTML = '<span style="color:var(--dim)">—</span>';
    }
...
document.getElementById('create-account-btn').addEventListener('click', async ()=>{
  ...
  const body = { username, password, categoryId, subcategoryId: subcategoryId || null, displayName: username, avatarColor };
...
function startEditCategory(cat){
  ...
  document.getElementById('cat-form-title').textContent = t('editCategoryTitle');
  document.getElementById('new-cat-name').value = cat.name;
  document.getElementById('new-cat-color').value = cat.color || '#00ff9c';
  document.getElementById('new-cat-webhook').value = cat.webhook_url || '';
  document.getElementById('create-cat-btn').textContent = t('saveChangesBtn');
  document.getElementById('cancel-edit-cat-btn').classList.remove('hidden-el');
}
function cancelEditCategory(){
  state.editingCategoryId = null;
  document.getElementById('cat-form-title').textContent = t('createCategoryTitle');
  document.getElementById('new-cat-name').value = '';
  document.getElementById('new-cat-color').value = '#00ff9c';
  document.getElementById('new-cat-webhook').value = '';
  document.getElementById('create-cat-btn').textContent = t('addBtn');
  document.getElementById('cancel-edit-cat-btn').classList.add('hidden-el');
}
document.getElementById('cancel-edit-cat-btn').addEventListener('click', cancelEditCategory);
...
document.getElementById('create-cat-btn').addEventListener('click', async ()=>{
  ...
  const webhook_url = document.getElementById('new-cat-webhook').value.trim() || null;
  if(!name){ errEl.textContent = 'Name required.'; return; }
  let error;
  if(state.editingCategoryId){
    ({ error } = await sb.from('categories').update({ name, color, webhook_url }).eq('id', state.editingCategoryId));
  } else {
    ({ error } = await sb.from('categories').insert({ name, color, webhook_url }));
  }
  if(error){ errEl.textContent = 'ERROR: ' + error.message; return; }
  cancelEditCategory();
  await refreshAllData();
...
/* -------- Edit Account modal -------- */
let editAccountTarget = null;

function updateEditAccSubcats(selectedId){
  const catId = document.getElementById('edit-acc-category').value;
  const subs = state.subcategories.filter(s=>s.category_id===catId);
  document.getElementById('edit-acc-subcategory').innerHTML = '<option value="">—</option>' + subs.map(s=>`<option value="${s.id}" ${s.id===selectedId?'selected':''}>${escapeHtml(s.name)}</option>`).join('');
}

function openEditAccountModal(acc){
  editAccountTarget = acc;
  document.getElementById('edit-acc-username').value = acc.username;
  document.getElementById('edit-acc-password').value = '';
  document.getElementById('edit-acc-business').value = acc.business || '';
  document.getElementById('edit-acc-color').value = acc.avatar_color || avatarFor(acc).color || '#00ff9c';

  const roleSel = document.getElementById('edit-acc-role');
  roleSel.innerHTML = state.roles.filter(r=>r.level<9 || r.id===acc.role_id)
    .map(r=>`<option value="${r.id}" ${r.id===acc.role_id?'selected':''}>${escapeHtml(r.name)} (lvl ${r.level})</option>`).join('');

  const th = state.threads.find(t2=>t2.client_id===acc.id);
  const catSel = document.getElementById('edit-acc-category');
  catSel.innerHTML = state.categories.map(c=>`<option value="${c.id}" ${th && th.category_id===c.id?'selected':''}>${escapeHtml(c.name)}</option>`).join('');
  updateEditAccSubcats(th ? th.subcategory_id : null);
  catSel.onchange = ()=> updateEditAccSubcats(null);

  document.getElementById('edit-acc-error').textContent = '';
  document.getElementById('edit-acc-success').textContent = '';
  document.getElementById('edit-account-modal').classList.add('visible');
}

function closeEditAccountModal(){
  document.getElementById('edit-account-modal').classList.remove('visible');
  editAccountTarget = null;
}
document.getElementById('edit-acc-cancel-btn').addEventListener('click', closeEditAccountModal);

document.getElementById('edit-acc-save-btn').addEventListener('click', async ()=>{
  const errEl = document.getElementById('edit-acc-error');
  const okEl = document.getElementById('edit-acc-success');
  errEl.textContent = ''; okEl.textContent = '';
  if(!editAccountTarget) return;

  const body = {
    targetId: editAccountTarget.id,
    username: document.getElementById('edit-acc-username').value.trim(),
    business: document.getElementById('edit-acc-business').value.trim(),
    avatarColor: document.getElementById('edit-acc-color').value,
    roleId: document.getElementById('edit-acc-role').value,
    categoryId: document.getElementById('edit-acc-category').value,
    subcategoryId: document.getElementById('edit-acc-subcategory').value || null,
  };
  ...
  try{
    ...
    json = await res.json();
  } catch(e){ errEl.textContent = 'NETWORK ERROR: ' + e.message; return; }
  if(!res.ok){ errEl.textContent = 'ERROR (' + res.status + '): ' + (json && json.error ? json.error : 'unknown'); return; }

  okEl.textContent = t('savedOk');
  await refreshAllData();
  renderSidebar();
  renderSettingsAll();
  setTimeout(closeEditAccountModal, 500);
});

/* -------- Maintenance: image cleanup -------- */
document.getElementById('cleanup-images-btn').addEventListener('click', async ()=>{
  const errEl = document.getElementById('cleanup-error');
  const okEl = document.getElementById('cleanup-success');
  errEl.textContent = ''; okEl.textContent = '';
  let res, json;
  try{
    res = await fetch('/api/cleanup-images', {
      method:'POST',
      headers:{ 'Authorization':'Bearer '+state.session.access_token }
    });
    json = await res.json();
  } catch(e){ errEl.textContent = 'NETWORK ERROR: ' + e.message; return; }
  if(!res.ok){ errEl.textContent = 'ERROR: ' + (json && json.error ? json.error : 'unknown'); return; }
  okEl.textContent = t('cleanupDone').replace('{n}', json.cleared != null ? json.cleared : 0);
});
...
async function openThread(threadId){
  ...
  if(isOwnThread(threadId)){ const mc = document.querySelector('.nav-item.mychat'); if(mc) mc.classList.add('active'); }
...
function renderChatHeader(threadId){
  ...
  const sc = state.subcategories.find(s=>s.id===th.subcategory_id);
  const acc = state.accounts.find(a=>a.id===th.client_id);
  document.getElementById('chat-meta').textContent = (cat?cat.name:'') + (sc?' / '+sc.name:'') + (acc && !acc.is_active ? '  [DISABLED]' : '');
...
function renderMessages(msgs, threadId){
  ...
  let lastDay = null, lastSenderKey = null, lastTs = 0;

  msgs.forEach(m=>{
    const mine = m.sender_id === state.profile.id;
    const day = fmtDay(m.created_at);
    if(day !== lastDay){
      container.appendChild(el(`<div class="day-sep">${escapeHtml(day)}</div>`));
      lastDay = day; lastSenderKey = null;
    }

    let bubbleCls = 'msg';
    if(fullView){ bubbleCls += m.visibility === 'internal' ? ' internal' : ' client-reply'; }
    else { bubbleCls += mine ? ' from-client' : ' client-reply'; }

    let rawName;
    if(fullView){ rawName = m.sender_username; }
    else { rawName = mine ? t('you') : t('staff'); }
    const displayName = fullView ? prettyName(rawName) : rawName;
    const badge = fullView ? (m.visibility==='internal' ? ' ' + t('internalNote') : ' ' + t('sentToClient')) : '';

    const senderKey = (fullView ? m.sender_id : (mine ? 'me' : 'other')) + '|' + m.visibility;
    const grouped = (senderKey === lastSenderKey) && (new Date(m.created_at).getTime() - lastTs < 3*60*1000);
    lastSenderKey = senderKey; lastTs = new Date(m.created_at).getTime();

    const { color, letter } = avatarForMessage(m, threadId, fullView);
    const rowCls = 'msg-row ' + (mine ? 'own' : 'other') + (grouped ? ' grouped' : '');

    const row = el(`<div class="${rowCls}">
      <div class="avatar ${grouped?'spacer':''}" style="${avatarStyle(color)}">${escapeHtml(letter)}</div>
      <div class="msg-col">
        <div class="${bubbleCls}">
          <div class="meta"><b style="color:${color}">${escapeHtml(displayName)}</b>${escapeHtml(badge)} <span class="time">· ${fmtTime(m.created_at)}</span></div>
          <div class="body"></div>
          ${fullView ? '<span class="del">✕</span>' : ''}
        </div>
      </div>
    </div>`);
    row.querySelector('.body').textContent = m.content;
    if(m.image_url){
      const img = el(`<img class="attach" src="${escapeHtml(m.image_url)}" />`);
      img.onclick = ()=> window.open(m.image_url, '_blank');
      row.querySelector('.msg').appendChild(img);
    }
    if(fullView){
      const delSpan = row.querySelector('.del');
      if(myPerm('can_delete')){
        delSpan.onclick = async ()=>{ const { error } = await sb.from('messages').delete().eq('id', m.id); if(error){ alert('ERROR: '+error.message); } };
      } else { delSpan.style.display = 'none'; }
    }
...
function subscribeToThread(threadId){
  ...
  state.msgChannel = sb.channel('messages-'+threadId)
    .on('postgres_changes', { event:'*', schema:'public', table:'messages', filter:'thread_id=eq.'+threadId }, ()=>{ loadMessagesForThread(threadId); })
    .subscribe();
}
...
function resolveVisibility(threadId, text){
  ...
  if(!own){
    ...
    if(/^!r\s+/i.test(cleanText)){ visibility = 'client'; cleanText = cleanText.replace(/^!r\s+/i, ''); }
  }
...
async function sendMessage(){
  ...
  const { error } = await sb.from('messages').insert({
    thread_id: threadId, sender_id: state.profile.id, sender_username: state.profile.username,
    content: cleanText.trim(), visibility
  });
  if(error){ alert('ERROR: ' + error.message); return; }
  input.value = '';
  await loadMessagesForThread(threadId);
...
async function sendImage(file){
  ...
  imageBtn.textContent = '...'; imageBtn.disabled = true;
  ...
  try{
    ...
    const { error } = await sb.from('messages').insert({
      thread_id: threadId, sender_id: state.profile.id, sender_username: state.profile.username,
      content: cleanText.trim(), image_url: imageUrl, visibility
    });
    if(error){ alert('ERROR: ' + error.message); return; }
    ...
    imageBtn.textContent = prevLabel; imageBtn.disabled = false;
...
document.getElementById('reply-toggle').addEventListener('change', (e)=>{ document.getElementById('reply-toggle-wrap').classList.toggle('on', e.target.checked); });
document.getElementById('chat-image-btn').addEventListener('click', ()=> document.getElementById('chat-image-input').click());
document.getElementById('chat-image-input').addEventListener('change', (e)=>{ const file = e.target.files && e.target.files[0]; if(file) sendImage(file); });
